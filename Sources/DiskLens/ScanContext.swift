...
enum ScanContext {
    ...
    static func systemPrompt(for model: AppModel,
        ...
        lines.append("")
        lines.append("ABSOLUTE RULE — PERSONAL FOLDERS:")
        lines.append("- NEVER suggest any of the following folders AS A WHOLE ITEM in the diskclean block, even if they are very large or marked 'verify-first'. DiskLens will drop whole-folder suggestions for these paths and the user will see nothing:")
        lines.append("    ~/Downloads, ~/Documents, ~/Desktop, ~/Movies, ~/Music, ~/Pictures, ~/Library, ~/Library/Mobile Documents (iCloud), /Users/<name>/Library (any user library root)")
        lines.append("- Most people keep important irreplaceable files in Downloads, Documents, and Desktop even though they shouldn't. Wiping the whole folder is never correct.")
        lines.append("- If one of these folders is large and needs trimming, WALK INTO IT: use the TOP FILES and TOP FOLDERS lists below to identify specific files or subfolders within it that look like installers (.dmg, .pkg), old zip archives, duplicate downloads, or clearly-cached content, and suggest those individual items instead.")
        lines.append("- If you cannot identify specific throwaway items inside a personal folder from the report, recommend in the prose that the user review it manually — but do NOT put the parent folder in the diskclean block.")
        lines.append("")
        lines.append("- STRONGLY PREFER paths inside the user's home folder that are owned by the user:")
        lines.append("    ~/Library/Caches, ~/Library/Developer (Xcode derived data, iOS device support, simulator data), ~/Library/Logs, ~/Library/Containers/*/Data/Library/Caches, ~/.Trash, individual old installers inside ~/Downloads")
